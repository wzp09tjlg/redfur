...
final class StickyHeaderPositioner {
    ...
    StickyHeaderPositioner(RecyclerView recyclerView) {
        ...
        mHeaderContainer = (FrameLayout) getRecyclerParent().findViewById(R.id.header_container);
    }
    ...
    void updateHeaderState(int firstVisiblePosition, Map<Integer, View> visibleHeaders,
        ...
        if (headerPositionToShow != lastBoundPosition) {
            if (headerPositionToShow == INVALID_POSITION ||
                checkMargins && headerAwayFromEdge(
                    headerToCopy)) { // We don't want to attach yet if header view is not at edge
                dirty = true;
                safeDetachHeader();
    ...
    void attachHeader(RecyclerView.ViewHolder viewHolder, int headerPosition) {
        ...
        if (mHeaderContainer == null) {
            mHeaderContainer = new FrameLayout(recyclerView.getContext());
            mHeaderContainer.setId(R.id.header_container);
            getRecyclerParent().addView(mHeaderContainer, new ViewGroup.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.WRAP_CONTENT));
        }

        mHeaderContainer.addView(currentHeader);
        ...
        mHeaderContainer.post(new Runnable() {
            @Override
            public void run() {
    ...
    private void detachHeader() {
        ...
        mHeaderContainer.removeView(currentHeader);
        currentHeader = null;
